...
//把data_pair_值传入点对中，保证其合理性，没有公共的起点
void Warp::InitPoints(QVector<QLine*>& data_pair_) {
...
void Warp::imageWarp(QImage* image_, int setgap)
{
	if (image_->isNull())
	{
		return;
	...
	image_->fill(Qt::white); //background color
	...
	for (int i = 0; i < count_width; i++)
	{
		...
		{
			vertices_new[i][j] = Warp_f(vertices_old[i][j]);
...
Vector2d Warp::Warp_f(const QPoint& p) const
{
	return Vector2d();
}

double Warp::dist(const QPoint& p1, const QPoint& p2)const {
	return sqrt((p1.x() - p2.x()) * (p1.x() - p2.x()) + (p1.y() - p2.y()) * (p1.y() - p2.y()));
}

int Warp::isInTriangle(Vector2d A, Vector2d B, Vector2d C, int x, int y) const
{
	...
	if ((signOfAB > -EPSILON && signOfCA > -EPSILON && signOfBC > -EPSILON)
		|| (signOfAB < EPSILON && signOfCA < EPSILON && signOfBC < EPSILON))
	{
		return 1;
	}
...
Vector2d Warp::cooridinates(Vector2d O, Vector2d A, Vector2d B, int x, int y) const
{
	Vector2d OA = A - O;
	Vector2d OB = B - O;
